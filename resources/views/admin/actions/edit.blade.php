...
@section('title', 'Éditer une action')

@section('content')
<div class="max-w-2xl mx-auto">
  <h1 class="text-3xl font-bold mb-6">Éditer l'action</h1>

  <form action="{{ route('admin.actions.update', $action) }}" method="POST" enctype="multipart/form-data"
    ...
    <div>
      ...
      <input type="text" name="title" value="{{ old('title', $action->title) }}"
    ...
    <div>
      ...
      <select name="icon"
        ...
        <option value="megaphone" {{ old('icon', $action->icon) === 'megaphone' ? 'selected' : '' }}>Mégaphone</option>
        <option value="book" {{ old('icon', $action->icon) === 'book' ? 'selected' : '' }}>Livre</option>
        <option value="graduation" {{ old('icon', $action->icon) === 'graduation' ? 'selected' : '' }}>Graduation</option>
        <option value="heart" {{ old('icon', $action->icon) === 'heart' ? 'selected' : '' }}>Cœur</option>
        <option value="users" {{ old('icon', $action->icon) === 'users' ? 'selected' : '' }}>Utilisateurs</option>
        <option value="shield" {{ old('icon', $action->icon) === 'shield' ? 'selected' : '' }}>Bouclier</option>
      </select>
    ...
    <div>
      ...
      @if($action->image_path)
        <div class="mb-3">
          ...
          <img src="{{ asset($action->image_path) }}" alt="{{ $action->title }}" class="h-24 rounded-lg border border-gray-200">
        </div>
      @endif
    ...
    <div>
      ...
      <input type="number" name="order" value="{{ old('order', $action->order) }}"
             class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent">
    </div>

    <div>
      <label class="flex items-center gap-2">
        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $action->is_active) ? 'checked' : '' }}>
