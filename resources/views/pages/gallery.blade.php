...
<section class="container-page py-12">
  ...
  <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    @forelse($galleryImages as $i => $photo)
      <figure data-cat="{{ $photo->category->name ?? 'Général' }}"
        ...
        <figcaption class="absolute inset-0 bg-gradient-to-t from-[var(--color-deep)]/90
          ...
          <span class="text-xs text-[var(--color-secondary)] font-semibold">{{ $photo->category->name ?? 'Général' }}</span>
          <span class="text-sm text-white font-medium">{{ $photo->title }}</span>
        </figcaption>
      </figure>
    @empty
      <div class="col-span-full text-center py-12">
        <p class="text-[var(--color-muted-foreground)]">Aucune image disponible pour le moment.</p>
      </div>
    @endforelse
  </div>

  <p class="text-center text-sm text-[var(--color-muted-foreground)] mt-12">
    📸 Espace réservé — les vraies photos et vidéos seront ajoutées prochainement.
  </p>
</section>
