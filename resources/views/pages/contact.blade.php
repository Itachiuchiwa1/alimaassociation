...
<section class="container-page py-16 grid lg:grid-cols-2 gap-10">
  ...
  <div class="space-y-6">
    ...
    <div class="bg-[var(--color-card)] rounded-3xl p-8 border border-[var(--color-border)] shadow-soft space-y-4">
      @php
        $contacts = [
          ['icon' => 'map', 'label' => 'Adresse',    'key' => 'address'],
          ['icon' => 'tel', 'label' => 'Téléphone',  'key' => 'phone'],
        ...
        <div class="flex gap-3">
          ...
          <div>
            ...
            <div class="text-sm text-[var(--color-muted-foreground)]">
              @php
                $setting = \App\Models\SiteSetting::where('key', $c['key'])->first();
                echo $setting?->value ?? 'À compléter';
              @endphp
            </div>
          </div>
        </div>
